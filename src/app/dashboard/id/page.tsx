...
import { Shield, CreditCard, AlertCircle, CheckCircle, Clock, Wifi, WifiOff } from 'lucide-react';
    ...
    async function fetchUser() {
      try {
        ...
        const res = await fetch('/api/users/me', {
          headers: token ? { 'x-user-id': JSON.parse(localStorage.getItem('league_user') || '{}').id } : {}
        });
        if (res.ok) {
      ...
      {isOffline && (
        <div className="mb-4 glass-card p-3 flex items-center gap-2 bg-yellow-500/20 border-yellow-500/50">
          ...
          <span className="text-yellow-400 text-sm">
            Offline Mode – Last synced {lastSynced ? Math.floor((Date.now() - lastSynced.getTime()) / 60000) : 0}m ago
          </span>
        </div>
      )}
      ...
      <div className={`glass-card p-6 relative overflow-hidden ${isLocked ? 'status-locked' : 'status-active'}`}>
        ...
        {isLocked && (
          <div className="absolute inset-0 bg-red-900/90 flex items-center justify-center z-20">
            <div className="text-center p-6">
              ...
              <a href="/dashboard/payments" className="mt-4 inline-block btn-primary bg-red-600 hover:bg-red-500">
                Pay Now
              </a>
            </div>
          </div>
          ...
          <div className={`w-20 h-20 rounded-full overflow-hidden border-2 ${!isLocked ? 'border-[#00F5FF]' : 'border-red-500'}`}>
            {user.photoUrl ? (
              ...
              <div className="w-full h-full bg-gray-700 flex items-center justify-center">
                <span className="text-2xl text-gray-400">{user.fullName?.charAt(0) || '?'}</span>
              </div>
            )}
          </div>
          <div className="flex-1">
            <h2 className={`text-xl font-bold ${!isLocked ? 'verified' : ''}`}>
              {user.fullName}
            </h2>
            <p className="text-secondary text-sm">{user.role}</p>
          </div>
          <div className={`px-3 py-1 rounded-full text-sm font-semibold ${!isLocked ? 'status-active' : 'status-locked'}`}>
        ...
        <div className="flex justify-center mb-4">
          <div className="glass-card p-4 w-48 h-48 flex items-center justify-center">
            <div className="text-center">
              {qrCodeUrl ? (
                <img src={qrCodeUrl} alt="Player QR Code" className="w-32 h-32 mx-auto" />
              ) : (
                <div className="w-32 h-32 mx-auto border-2 border-dashed border-[#00F5FF] rounded-lg flex items-center justify-center">
                  <span className="text-[#00F5FF] text-4xl font-bold">...</span>
                </div>
              )}
              {!isOffline && (
                <div className="flex items-center justify-center gap-1 mt-2 text-xs text-secondary">
                  <Clock size={12} />
                  <span>Refreshes in {Math.ceil(countdown / 1000)}s</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <Shield size={18} className={user.isInsured ? 'text-green-400' : 'text-red-400'} />
            <div>
              <p className="text-xs text-secondary">Insurance</p>
              <p className={user.isInsured ? 'text-green-400' : 'text-red-400'}>{insuranceStatus}</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            {user.backgroundCheckStatus === 'CLEAR' ? (
              <CheckCircle size={18} className="text-green-400" />
            ) : (
              <AlertCircle size={18} className="text-yellow-400" />
            )}
            <div>
              <p className="text-xs text-secondary">Background Check</p>
              <p className={user.backgroundCheckStatus === 'CLEAR' ? 'text-green-400' : 'text-yellow-400'}>
          ...
          <div className="flex items-center gap-3">
            <CreditCard size={18} className="text-[#00F5FF]" />
            <div>
              <p className="text-xs text-secondary">Player ID</p>
              <p className="font-mono text-[#00F5FF] text-sm">{user.id}</p>
            </div>
          </div>
        </div>
      </div>
