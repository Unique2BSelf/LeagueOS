...
import { logout, syncStoredUser } from '@/lib/client-auth'
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-white/50">Manage your league activities from one place.</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="glass-card p-4"><div className="text-2xl font-bold text-cyan-400">0</div><div className="text-white/50 text-sm">Matches Played</div></div>
          <div className="glass-card p-4"><div className="text-2xl font-bold text-green-400">0</div><div className="text-white/50 text-sm">Goals</div></div>
          <div className="glass-card p-4"><div className="text-2xl font-bold text-purple-400">0</div><div className="text-white/50 text-sm">Teams</div></div>
          <div className="glass-card p-4"><div className="text-2xl font-bold text-orange-400">$0</div><div className="text-white/50 text-sm">Balance</div></div>
        </div>

        <div className="space-y-4">
          {menuGroups.map((group) => (
            <div key={group.id} className={`glass-card border-l-4 ${group.color}`}>
              <button onClick={() => toggleGroup(group.id)} className="w-full flex items-center justify-between p-4 hover:bg-white/5 transition-colors">
                <div className="flex items-center gap-3">
                  <group.icon className="w-5 h-5 text-white/70" />
                  <span className="text-lg font-semibold text-white">{group.label}</span>
                </div>
                {expandedGroups[group.id] ? <ChevronDown className="w-5 h-5 text-white/50" /> : <ChevronRight className="w-5 h-5 text-white/50" />}
              </button>

              {expandedGroups[group.id] && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 px-4 pb-4">
                  {group.items.map((item) => (
                    <Link key={item.href} href={item.href} className="glass-card p-4 hover:bg-white/10 transition-colors group text-center">
