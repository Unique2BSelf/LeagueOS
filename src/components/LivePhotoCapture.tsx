          ...
          {e2eEnabled && (
            <div className="rounded-lg border border-cyan-500/30 bg-cyan-500/10 p-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-medium text-cyan-200">E2E test capture</p>
                  <p className="text-xs text-cyan-100/80">Dev-only bypass for automated browser tests.</p>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    const testPhoto = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAuMBg6V2VN8AAAAASUVORK5CYII=';
                    setCapturedPhoto(testPhoto);
                    setVerificationState('verified');
                    setLivenessPassed(true);
                    setFaceError(null);
                    onCapture(testPhoto);
